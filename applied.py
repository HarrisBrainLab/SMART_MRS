...
def add_progressive_motion_artifact(fids, time, num_affected_trans=None, echo=False):
    ...
    fids, trans_affected = add_freq_drift_linear(time, fids, freq_offset_var=off_var, freq_shift=slope_var, start_trans=start_trans, num_trans=num_affected_trans, echo=echo)
...
def add_subtle_motion_artifact(fids, time, echo=False):
    ...
    fids, trans_fs = add_freq_shift(time, fids, freq_var=freq_shift_var, cluster=False, num_trans=num_affected_trans, echo=echo)
    fids, trans_ps = add_zero_order_phase_shift(fids, phase_var=phase_shift_var, cluster=False, num_trans=num_affected_trans, echo=echo)
...
def add_disruptive_motion_artifact(fids, time, ppm, mot_locs=None, nmb_motion=1, echo=False):
    ...
    fids, mot_locs = add_linebroad(fids, time=time, amp=amp, damp=damp, mot_locs=mot_locs, nmb_motion=nmb_motion, echo=echo)  
    fids, mot_locs = add_baseline(fids, ppm, base_profile=motion_profile, cluster=False, nbase_locs=mot_locs, num_trans=nmb_motion, echo=echo)
    
    return fids, mot_locs


def add_lipid_artifact(fids, ppm, edited=1, cluster=False, lp_locs=None, nmb_lps=1, echo=False):
    '''
    ...
    fids, lp_locs = add_nuisance_peak(fids, ppm, peak_profile=lipid_profile, cluster=cluster, np_locs=lp_locs, num_trans=nmb_lps, echo=echo)
